...
resource "routeros_interface_wireguard_peer" "wireguard-peer" {
  ...
  allowed_address = flatten(each.value.subnets, ["${split(".", cidrhost(var.default_cidr, 0))[0]}.${split(".", cidrhost(var.default_cidr, 0))[1]}.${var.vpn_interface_subnet}.${each.value.id}/32"])
}
